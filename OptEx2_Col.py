...
plt.text(.23,0.3,'x(t)= 1 - t')
plt.text(.33,-.9, 'u(t)= -1')
plt.text(.43,1.1,'p(t)= 2 - t')
s = 'Final cost is: J = '+str(J)
#s = 'Final cost is: J = '+str(np.round(J,5))
plt.text(0.41,.8,s)
plt.xlabel('time')
plt.ylabel('states')
plt.grid()
plt.legend(framealpha=1, shadow=True)
plt.title('colnew: Optimal control Problem of memo0 Ex2')
plt.savefig('optEx2_Col.png')
plt.show()
